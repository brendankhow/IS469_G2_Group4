...
export async function POST(
  ...
  try {
    ...
    const parseResponse = await fetch('http://localhost:8000/chat/parse_schedule', {
      method: 'POST',
      headers: {
    ...
    if (!parseData.slots || parseData.slots.length === 0) {
      return NextResponse.json(
        { error: "Could not understand the scheduling request. Please try again with specific dates and times (e.g., 'Monday and Tuesday at 9am')." },
        { status: 400 }
      )
    }
    ...
    return NextResponse.json({
      ...
      message: parseData.ai_message || `I've scheduled interview slots and sent them to ${application.profiles.name || "the candidate"}.`,
